...
:::note
This solution was a little hacky because it required shinyjs for the dateInput to properly update. I [opened up an issue here](https://github.com/rstudio/shiny/issues/2798) since I think this is not the most intuative answer.
:::
...
We can use `updateSelectInput` to track the `selectInput` with id `county` and by appropriately filtering change it's values based on `input$state`. Additionally for the label argument in `updateSelectInput`, rather than supply a string we can use the reactive `label()`. This reactive uses `switch` to appropriately change the label text when either, `Alaska`, `Louisiana`, or any other state is supplied.
...
Similar to the previous exercise we will use `updateSelectInput` to filter the `country` options based on the selected `continent`. Rather than perform this computation inside the input, we are doing it inside a reactive, `selected_data()` so that we can use this same reactive without `output$data` rather than repeat the computation twice.
...
We initially set the choices of `input$continent` to `""` which results in `input$country` not filtering.

:::note
Initially setting the choices to `c("", as.character(continents))` allows the user to see all the Country options prior to a continent being selected. That said, once a continent is selected this `""` option is no longer available. We can supply a default value like `All` and change the logic so that when `All` is selected we don't filter our data, but I'm not sure this is what the question was asking.
:::
...
We can use two `observeEvent` functions that change the `slider` based on `numeric` and vice versa. To keep the values in sync we can use the `isolate` to....... (TODO: not really sure what isolate's function is here seemingly works okay without it....)
