...
Conversely, we can first `fct_lump` first, which will make the most frequently seen factor `Other`. Now when we apply `fct_infreq()` the first level will be `Other` and not `Laceration` as above.
