...
We are given the following list of supplied code:
```{r, eval=FALSE}
...
For the UI function we will need the `textInput` for the user to dynamically input their text and `textOutput` to generate the text we want the user to see given their input. The corresponding server function to `textOutput` is `renderText` which we can use to compose the element we've named "greeting".
...
The error here is that we need to specify `input$x`. Shiny accesses the ID's of elements within a read only object called `input`. By writing `x`, we are looking for element `x` which doesn't exist in the Shiny environment, as it only exists within the object `input`.
...
We can add another `slideInput` with ID `y`. Then can use both  using both `input$x` and `input$y` to calculate `output$product`.
