...
The error here is that we need to specify `input$x`. Shiny accesses the ID's of elements within a list called `input`. By writing `x`, we are looking for element `x` which doesn't exist in the Shiny environment, as it only exists within the `input` list.
...
We can add another `slideInput` with ID `y`. Thenwe can use both  using both `input$x` and `input$y` to calculate `output$product`.
